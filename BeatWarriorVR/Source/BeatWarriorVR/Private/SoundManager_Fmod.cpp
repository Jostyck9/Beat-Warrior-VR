...
{
	FMOD_RESULT result = _system->createSound(pathToFile.c_str(), FMOD_LOOP_OFF | FMOD_CREATESAMPLE, 0, &_sound);
	return result;
}
...
int SoundManager_Fmod::loadSoundFromMemory(char* memoryPtr, unsigned int memorySize)
{
	...
	FMOD_RESULT result = _system->createSound(memoryPtr, FMOD_OPENMEMORY | FMOD_LOOP_OFF | FMOD_CREATESAMPLE, &sndinfo, &_sound);
	return result;
}
